...
interface PopularMenProps {
  data_men_product: Product[];
}

const PopularMen: React.FC<PopularMenProps> = ({ data_men_product }) => {
  ...
  return (
    ...
    >
      ...
      <p className="text-center text-lg mx-1 mt-8 text-gray-600 mb-12 font-mono font-bold">
        Discover our top trending sports tops, perfect for any activity. Whether you're aiming for high performance or casual comfort, we have something for you ✨.
      </p>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
        {data_men_product.map((item) => (
          <div
            key={item.id}
          ...
          >
            <div className="relative w-full h-80 overflow-hidden rounded-lg mb-5 md:mb-0 lg:mb-5 md:mr-5 lg:mr-0">
              <img
                src={item.image}
                alt={item.name}
      ...
      <hr className="border-t-2 border-roseGold my-16 w-1/2 mx-auto" />  
    </div>
  );
}
